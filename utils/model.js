(() => {
  ...
  const rule = (source = '#28A745') => ({ id: crypto.randomUUID(), enabled: true, source, target: '#2979FF', threshold: C.THRESHOLD_DEFAULT });
  function validate(input) {
    ...
    function checkProfile(p) {
      ...
      const ids = new Set();
      return { enabled: p.enabled, name: p.name, rules: p.rules.map(r => {
        ...
        return { id: r.id, enabled: r.enabled, source: Color.rgbToHex(Color.hexToRgb(r.source)), target: Color.rgbToHex(Color.hexToRgb(r.target)), threshold: r.threshold };
      }) };
    }
    ...
    for (const [host, p] of Object.entries(input.profiles)) {
      ...
      profiles[host] = checkProfile(p);
    }
    return { version: C.VERSION, settings: { enabled: input.settings.enabled }, globalProfile: checkProfile(input.globalProfile), profiles };
  }
  ...
  P.Model = { profile, defaults, rule, validate, effective, importJSON, exportJSON };
})();
